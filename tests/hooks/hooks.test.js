...
async function runTests() {
  ...
  if (
    await asyncTest('exits 0 even with isolated empty HOME', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-iso-start-${Date.now()}`);
  ...
  if (
    await asyncTest('skips template session content', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-tpl-start-${Date.now()}`);
  ...
  if (
    await asyncTest('injects real session content', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-real-start-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('reports learned skills count', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-skills-start-${Date.now()}`);
  ...
  if (
    await asyncTest('creates or updates session file', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-session-create-${Date.now()}`);
  ...
  if (
    await asyncTest('writes project, branch, and worktree metadata into new session files', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-session-metadata-${Date.now()}`);
  ...
  if (
    await asyncTest('annotates active session file with compaction marker', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-compact-annotate-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('compaction log contains timestamp', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-compact-ts-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    test('InsAIts hook is opt-in and scoped to high-signal tool inputs', () => {
      ...
      assert.ok(insaitsHook.description.includes('CC4PM_ENABLE_INSAITS=1'), 'InsAIts hook should document explicit opt-in');
      assert.ok(
  ...
  if (
    test('plugin.json does NOT have explicit hooks declaration', () => {
      ...
      // See: https://github.com/istarwyh/cc4pm/issues/103
  ...
  if (
    test('observer-loop uses a configurable max-turn budget with safe default', () => {
      ...
      assert.ok(observerLoopSource.includes('CC4PM_OBSERVER_MAX_TURNS'), 'observer-loop should allow max-turn overrides');
      assert.ok(observerLoopSource.includes('max_turns="${CC4PM_OBSERVER_MAX_TURNS:-10}"'), 'observer-loop should default to 10 turns');
      assert.ok(!observerLoopSource.includes('--max-turns 3'), 'observer-loop should not hardcode a 3-turn limit');
      assert.ok(observerLoopSource.includes('CC4PM_SKIP_OBSERVE=1'), 'observer-loop should suppress observe.sh for automated sessions');
      assert.ok(observerLoopSource.includes('CC4PM_HOOK_PROFILE=minimal'), 'observer-loop should run automated analysis with the minimal hook profile');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    await asyncTest('detect-project writes project metadata to the registry and project directory', async () => {
      ...
      try {
        ...
        spawnSync('git', ['remote', 'add', 'origin', 'https://github.com/example/cc4pm-test.git'], { cwd: repoDir, stdio: 'ignore' });
        ...
        assert.strictEqual(metadata.remote, 'https://github.com/example/cc4pm-test.git', 'project.json should include the sanitized remote');
  ...
  if (await asyncTest('observe.sh skips automated sessions before project detection side effects', async () => {
    ...
    const cases = [
      ...
      },
      {
        ...
        env: { CLAUDE_CODE_ENTRYPOINT: 'cli', CC4PM_HOOK_PROFILE: 'minimal' }
      },
      {
        name: 'cooperative skip env',
        env: { CLAUDE_CODE_ENTRYPOINT: 'cli', CC4PM_SKIP_OBSERVE: '1' }
      },
      {
      ...
      },
      {
        ...
        env: {
          ...
          CC4PM_OBSERVE_SKIP_PATHS: ' observer-sessions , .claude-mem '
        },
  ...
  if (
    await asyncTest('only annotates *-session.tmp files, not other .tmp files', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-compact-glob-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('handles no active session files gracefully', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-compact-nosession-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 with empty sessions directory (no recent sessions)', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-start-empty-${Date.now()}`);
      fs.mkdirSync(path.join(isoHome, '.claude', 'sessions'), { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
  ...
  if (
    await asyncTest('does not inject blank template session into context', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-start-blank-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('annotates only the newest session file when multiple exist', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-compact-multi-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('does not inject empty session file content into context', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-start-empty-file-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('summary omits Files Modified and Tools Used when none found', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-notools-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('reports available session aliases on startup', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-start-alias-${Date.now()}`);
      fs.mkdirSync(path.join(isoHome, '.claude', 'sessions'), { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('parallel compaction runs all append to log without loss', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-compact-par-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 when sessions path is a file (not a directory)', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-start-blocked-${Date.now()}`);
  ...
  if (
    await asyncTest('excludes session files older than 7 days', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-start-7day-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('injects newest session when multiple recent sessions exist', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-start-multi-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('does not inject content when session file is unreadable', async () => {
      ...
      const isoHome = path.join(os.tmpdir(), `cc4pm-start-unreadable-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 when session file is read-only (appendFile fails)', async () => {
      ...
      const isoHome = path.join(os.tmpdir(), `cc4pm-compact-ro-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('logs warning when existing session file lacks Last Updated field', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-end-nots-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('extracts user messages from role-only format (no type field)', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-role-only-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('logs "Transcript not found" for nonexistent transcript_path', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-notfound-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('extracts tool name and file path from entry.name/entry.input (not tool_name/tool_input)', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-r70-entryname-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('shows selection prompt when no package manager preference found (default source)', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-r71-ss-default-${Date.now()}`);
  ...
  if (
    await asyncTest('extracts user messages from entries where only message.role is user (not type or role)', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-msgrole-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('skips user messages with numeric content (non-string non-array branch)', async () => {
      ...
      const isoHome = path.join(os.tmpdir(), `cc4pm-r81-numcontent-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
      const transcriptPath = path.join(isoHome, 'transcript.jsonl');
  ...
  if (
    await asyncTest('collects tool name from entry with tool_name but non-tool_use type', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-r82-toolname-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('preserves file when marker present but regex does not match corrupted template', async () => {
      const isoHome = path.join(os.tmpdir(), `cc4pm-r82-tmpl-${Date.now()}`);
      const sessionsDir = path.join(isoHome, '.claude', 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
