...
预览和完整检查共用生成目录。运行 `site:check` 前先停止该目录的 `dev:site`，完成检查后再启动预览，避免两种地址的构建互相覆盖。
...
- 构建拒绝警告，检查全部站内链接和锚点、CSS 资源、每课唯一 H1、Markdown 输出、`llms.txt`、导航和源文件哈希。课件代码块中的 Hugo shortcode 示例只在生成层转义；构建后逐课比较 Markdown 代码块与原文，并验证网页中的 shortcode 示例仍可原样复制。
