...
- User says "configure cc4pm", "install cc4pm", "setup everything claude code", or similar
...
1. **Via Plugin**: `/plugin install cc4pm` — the plugin loads this skill automatically
2. **Manual**: Copy only this skill to `~/.claude/skills/configure-cc4pm/SKILL.md`, then activate by saying "configure cc4pm"
...
```bash
rm -rf /tmp/cc4pm
git clone https://github.com/istarwyh/cc4pm.git /tmp/cc4pm
```

Set `CC4PM_ROOT=/tmp/cc4pm` as the source for all subsequent copy operations.
...
```bash
cp -r $CC4PM_ROOT/skills/<skill-name> $TARGET/skills/
```
...
cp -r $CC4PM_ROOT/rules/common/* $TARGET/rules/

# Language-specific rules (flat copy into rules/)
cp -r $CC4PM_ROOT/rules/typescript/* $TARGET/rules/   # if selected
cp -r $CC4PM_ROOT/rules/python/* $TARGET/rules/        # if selected
cp -r $CC4PM_ROOT/rules/golang/* $TARGET/rules/        # if selected
```
...
**Critical**: Only modify files in the installation target (`$TARGET/`), NEVER modify files in the source cc4pm repository (`$CC4PM_ROOT/`).
