...
所有课程菜单、下一步导航、选择题都必须调用 `AskUserQuestion` 工具展示选项；不要让用户输入数字、回复编号或手动键入选项序号。每个问题最多展示 4 个选项；如果选项超过 4 个，先用分组问题缩小范围，再继续追问。

当 lesson 文件的 `## 下一步` 列出选项时，从每条选项提炼 1-5 个词作为 `AskUserQuestion` 的 `option.label`，其余说明写入 `description`，不要把编号菜单直接输出给学习者。
...
调用 `AskUserQuestion` 提问：`{name}，你想怎么开始？`
...
- **Session 分离**：课程中涉及实操练习（如 `/bmad-*`、`/bmad-agent-*` 等命令）时，必须明确告诉用户"在新的 Claude Code session 中执行"，而不是在当前教学 session 中执行。这些命令会启动 Skill 或 Agent 接管对话，会中断教学流程。当前 session 始终保持教学教练角色
...
调用 `AskUserQuestion` 提问：`你想采用哪种学习方式？`

- header: `学习方式`
- options:
  - `从头开始`：推荐首次学习，从第一课开始循序渐进地学习
  - `跳到阶段`：选择阶段 1-5 中的特定阶段开始
  - `查看进度`：查看已完成课程和当前位置
  - `退出学习`：结束本次教学 session

如果用户选择 `跳到阶段`，继续调用 `AskUserQuestion` 进行阶段选择；不要要求用户输入阶段编号。阶段选择超过 4 个选项时，先让用户选择 `阶段 1-2`、`阶段 3`、`阶段 4`、`阶段 5`，再按需追问具体课程。
...
| 3.1 | Status Line——你的实时仪表盘 | status-line、token-monitoring、claude-hud-statusline、statusline-command |
...
| 5.1 | 上下文治理分层：Memory、Rules 与 AGENTS.md | context-governance-layers、agents-md、memory-boundaries、rules-paths |
...
| 9.1 | 环境变量完全手册 | configuring-env-two-methods、non-anthropic-model-compat-env、model-selection-routing-env、network-proxy-env-vars |
...
| 12.1 | BMad 完整工作流：四个阶段与三条轨道 | bmad-four-phases、bmad-planning-tracks、bmad-help-skill、bmad-fresh-chats |
