...
10. **AI 味自检（强制）** — 跑"提交前自检"章节列出的 grep 命令，按"形式回声检查"表数频率。任一类超上限必须重写。**跳过此步 = 必有 AI 味。**
...
- 超过 3 个动图必须在读者评审中逐个证明必要性；否则宁可删掉。
...
**生成后验证**：
- 用 grep 统计：`grep -o '<svg' file.html | wc -l` 和 `grep -oE '<animate|<animateMotion|<animateTransform' file.html | wc -l`。
...
如果文章包含 2 个以上 SVG 动图，必须额外让至少 1 个目标读者检查图文衔接。评审问题：
