...
7.  **目标读者 subAgent 评审（强制）** — 文章成稿后，根据主题 spawn 1–3 个目标读者 subAgent 代入阅读，并要求他们只反馈"哪里看不懂、哪里不信、哪里没获得感、哪里像 AI 写的"。详见"目标读者评审闭环"章节。
8.  **合理润色** — 汇总 subAgent 意见，只采纳能提升理解、信任、获得感、转发欲的建议；不得为了迎合所有读者而稀释文章主张。
9.  **AI 味自检（强制）** — 跑"提交前自检"章节列出的 grep 命令，按"形式回声检查"表数频率。任一类超上限必须重写。**跳过此步 = 必有 AI 味。**
10.  **包裹根 section** + 追加 `<mp-style-type>` 标记。
11.  **保存为 `.wechat.html`** 至 `~/wechat-exports/`。
