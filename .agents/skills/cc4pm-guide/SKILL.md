...
```
   ...
   ╚██████╗╚██████╗     ██║██║     ██║ ╚═╝ ██║
    ╚═════╝ ╚═════╝     ╚═╝╚═╝     ╚═╝     ╚═╝
              ...
              Codex FOR PRODUCT MAKERS
              AI-Driven Product Lifecycle OS
```

### 2. 认识用户（选择式交互）

**先检查已有记录**：读取 `memory/user_profile.md`，如果文件存在且包含有效的用户画像（有身份、学习目标等字段），则**跳过以下所有问题**，直接使用已有画像。在欢迎信息中说明"欢迎回来，{name}！上次你的学习目标是 {goal}，我们继续吧。"然后跳到第 4 步（AI 主理人介绍）。

只有在没有找到已有画像时，才执行以下提问流程。
...
如果 `memory/user_profile.md` 已存在，更新其中的字段（如学习目标、当前项目等）；如果不存在，则创建新文件。格式如下：
```markdown
---
name: user_profile
...
- **Session 分离**：课程中涉及实操练习（如 `/bmad-*`、`/bmad-agent-*` 等命令）时，必须明确告诉用户"在新的 Codex session 中执行"，而不是在当前教学 session 中执行。这些命令会启动 Skill 或 Agent 接管对话，会中断教学流程。当前 session 始终保持教学教练角色
...
- **`configure_mcp`**：提示 "检测到 QMD 已安装且有索引，但尚未配置 MCP。建议在 ~/.Codex.json 的 mcpServers 中添加 `\"qmd\": {\"command\": \"qmd\", \"args\": [\"mcp\"]}`，配置后 Codex 可直接搜索你的本地文档。" 然后继续教学流程，不阻塞。
...
- 掌握从创意到上线的完整产品开发流程
- 学会使用 AI 工具（Codex、MCP 服务器、绘图工具等）——需要的工具我来帮你装好
- 理解产品主理人的思维方式和工作方法
- 在真实项目中应用这些技能
...
- **产品思维**：从用户需求出发的产品设计方法
- **AI 工具使用**：Codex、MCP 服务器、绘图工具等的实战应用
...
- 完整学习：约 90-120 分钟（5 个阶段，26 节课 + 36 个补充课）
...
### 阶段 1：Codex 基础入门（10 节课）

| 编号 | 课名 | 核心知识点 |
|------|------|-----------|
| 1 | cc4pm 全景：四大模块与产品思维 | agent-loop、four-modules、cc4pm-overview、bmm-cis-wds-engineering |
| 1.1 | Codex 工作原理——源码级深度解析 | cc-internals、agent-loop-11-steps、system-prompt-assembly、tool-system-50plus |
...
| 4 | 快速上手：第一次实操 | prompt-precision、rich-input、screenshot-paste、Codex-interview |
| 4.1 | 压力光谱——用大厂 PUA 话术驱动 AI | pua-pressure-spectrum、three-red-lines、13-pua-flavors、pressure-level-selection |
| 5 | AGENTS.md：项目的 AI 记忆 | Codex-md、file-hierarchy、six-section-template、memory-md |
...
| 7.1 | Agent Teams——多 Codex 协作团队 | agent-teams、lead-teammates、iterm2-integration、multi-role-discussion |
...
| 13.1 | 网站情报工具——竞品分析的实战利器 | website-traffic-analysis、seo-audit、tech-stack-detection、adsense-detection |
...
| 23.6 | CLIProxyAPI 实战：用 Gemini 免费额度驱动 Codex | cliproxyapi、gemini-oauth-free-tier、api-proxy-config、multi-account-rotation |
...
| 24.6 | cc-connect 实战：通过微信远程控制 Codex | cc-connect、ilink-protocol、wechat-bot-gateway、remote-Codex-control |
| 25 | 完整项目实战：从零到发布 | full-workflow、key-handoff-points、project-launch-checklist、agent-collaboration-panorama |
| 25.1 | 开发者工作流：从 Issue 到 PR 的完整闭环 | issue-driven-workflow、multi-workspace-Codex、unrestricted-mode、external-ai-review |
