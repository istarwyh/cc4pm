...
- ❌ "最推荐的方式是让 Codex 自己装自己——这听起来像绕口令，但确实是最不踩坑的路"
- ✅ "让 Codex 自己装自己。把下面这句话发给 Codex，它会 fetch 官方 INSTALL.md，自己跟着步骤装"
- ❌ "最香的一点是不用记 cron 表达式"
- ✅ "不用记 cron 表达式"
- ❌ "这个功能我用得最频繁"
- ✅ 直接描述功能，不加个人使用频率
...
实例：
- 主题"工具配置交给 AI" → 把"手动改 TOML 第 N 行"重写为"告诉 Codex '把我的 wxid 加到管理员'，它自己改字段"
...
句子级和词汇级的 AI 味相对容易发现，真正危险的是**结构层面的简化**——它让文章看起来完整，但实际上剥夺了读者判断的空间。

#### 框架变终点

把分析框架（如"三种场景"、"两类用户"、"四个阶段"）当成文章骨架，结果框架变成了终点而不是起点。

- ❌ 文章以"三种场景 → 三个模板"收尾，隐含"选完场景就配完了"
- ✅ 框架只是粗粒度起点，文章还要呈现框架覆盖不到的判断细节

自检：文章的核心主张是什么？框架是在服务这个主张，还是替代了它？

#### 主角写成配角

当文章介绍了一个工具/方法/原则，它在文章中的位置应该与它的实际重要性一致。如果某个东西是"真正的解"，它应该出现在文章高位，而不是最后的"此外还有"。

- ❌ 核心工具放在文章末尾作为补充选项（"如果你嫌麻烦，也可以用 XXX"）
- ✅ 先讲为什么需要它，再讲它怎么用，模板/手动配置作为补充参考

#### 事实夸大（最容易漏掉）

"不配会报错"、"一定会失败"这类强化语气的断言，往往比实际情况夸张。真实情况常常是"不配会悄悄失效，症状难以追踪"——后者更准确，也更能说服读者。

在写任何"必须"、"否则报错"、"一定会"之前，先确认：这是立即可见的失败，还是静默的性能/稳定性问题？两者的描述方式完全不同。

#### 金句是最后才认真写的——但不应该

金句需要提出反直觉视角，这要求对内容有足够深的理解。如果写完全文才开始想金句，通常只能写出总结型的句子（"X 很重要，要重视 X"）。

金句应该在写作过程中自然冒出——当你真正理解了某个反直觉的事实，那个句子会主动跳出来。如果最后逼自己想金句，说明写作过程中思考深度不够。
...
## 语言规范
- 只有 logo (CC4PM) 和专有名词 (Codex, Anthropic) 用英文。
