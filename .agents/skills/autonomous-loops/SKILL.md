...
Codex 的所有循环模式，底层思想只有一个：**把 Codex 当成 Unix 工具用**。

```bash
while true; do
  Codex -p "读取任务文件，继续下一步..."
  if [ $? -eq 0 ]; then
    break
  fi
done
...
每次新开一个进程，靠文件通信。无状态、可组合、可管道化——和 `grep`、`sed`、`awk` 没有本质区别。区别只在于 Codex 能理解自然语言并自主决策下一步。

这是 Ralph Loop 的核心思想：让 Codex 不停地自我迭代，直到任务完成。
...
**cc4pm's built-in persistent loop.** A session-aware REPL that calls `Codex -p` synchronously with full conversation history.
