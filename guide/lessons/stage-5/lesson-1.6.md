...
> 1. 先在微信里发 `/whoami` 获取你的 wxid
> 2. 把 wxid 填入 `admin_from` 字段（多个管理员用逗号分隔）
> 3. `allow_from` 控制谁能使用 bot，留空表示不限制；`admin_from` 控制谁能执行敏感命令
>
> ```toml
> [[projects.platforms]]
> type = "weixin"
> allow_from = "wxid_abc123,wxid_def456"   # 只允许这两个用户使用
> admin_from = "wxid_abc123"               # 只有第一个是管理员
> ```
...
**Q: `/dir` 提示需要管理员权限怎么办？**
A: `/dir` 是敏感命令，需要管理员权限。先发 `/whoami` 获取你的 wxid，然后在 config.toml 的 `admin_from` 字段填入该 wxid，重启服务即可。
