...
PreToolUse（工具执行前）— 7 个 Hook
  ...
  ↓ insaits-security-wrapper.js：AI 安全监控（需 CC4PM_ENABLE_INSAITS=1）
...
| insaits-security-wrapper.js | PreToolUse | AI 安全监控：检测凭证泄露、提示注入、行为异常（23 种异常类型） | 需开启 `CC4PM_ENABLE_INSAITS=1` |
...
**关键设计**：
- `run-with-flags.js` 是一个**条件执行框架**——根据 `CC4PM_HOOK_PROFILE` 决定是否运行
...
```
CC4PM_HOOK_PROFILE=minimal    → 只运行 session-start/end（4 个 Hook）
CC4PM_HOOK_PROFILE=standard   → 运行大部分 Hook（默认，17 个）
CC4PM_HOOK_PROFILE=strict     → 运行全部 Hook（21 个）

CC4PM_DISABLED_HOOKS=post:edit:format,stop:cost-tracker
  → 单独禁用指定的 Hook

CC4PM_ENABLE_INSAITS=1
...
**环境变量微调**：
```
CC4PM_QUALITY_GATE_FIX=1     → 自动修复格式问题（默认行为）
CC4PM_QUALITY_GATE_STRICT=1  → 严格模式，Lint 警告也阻止
```
...
Level 3: 环境变量精确控制
  → CC4PM_HOOK_PROFILE=minimal  （4 个核心 Hook：session-start/end + evaluate + cost）
  → CC4PM_HOOK_PROFILE=standard （17 个 Hook，默认值）
  → CC4PM_HOOK_PROFILE=strict   （全部 21 个 Hook）
  → CC4PM_DISABLED_HOOKS=hook1,hook2  （精确禁用特定 Hook）
  → CC4PM_ENABLE_INSAITS=1            （开启 AI 安全监控）
  → CC4PM_QUALITY_GATE_STRICT=1       （质量门禁严格模式）
...
# 禁用特定 Hook
export CC4PM_DISABLED_HOOKS=suggest-compact
```
...
A: 可以通过 CC4PM_HOOK_PROFILE 调整严格程度（minimal/standard/strict）。也可以在 hooks.json 中禁用特定 Hook。关键是找到"质量"和"速度"的平衡点——对于 MVP 阶段可以用 minimal，产品稳定后切换到 strict。
...
---
*阶段 4 | Lesson 23/26 (阶段内 3/3) | 上一课: Lesson 22.4 - TDD Prompt 模板 | 下一课: Lesson 23.1 - Harness 设计哲学*
