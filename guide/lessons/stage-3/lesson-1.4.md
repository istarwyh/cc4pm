...
- 能为自己的产品选择并表达一种排版风格
- 会用 `kill-ai-slop` 扫描代码信号、解释命中原因，并在批准范围内做最小修复
...
### 用 `kill-ai-slop` 把审美判断变成可审查流程

前面的清单告诉你“要看什么”，[`kill-ai-slop`](https://github.com/yetone/kill-ai-slop) 则把这套判断变成一个 Agent Skill：先扫描代码，再逐条解释，最后只在你批准的范围内修改。
...
| 信号 | 常见代码形态 | 为什么值得检查 | 干净的修复方向 |
|------|-------------|---------------|---------------|
| 靛蓝到紫色渐变 | `from-indigo-*` + `to-violet-*` | 很多 AI 落地页共享同一套默认配色 | 回到已有品牌强调色 |
| 渐变裁切标题 | `bg-clip-text text-transparent` | 视觉冲击依赖效果，而不是字号与间距 | 使用实色文字建立层级 |
| 彩色左边框提示框 | `border-l-4` 重复出现 | 普通信息被包装成一叠装饰性 callout | 只保留真正需要强调的 aside |
| emoji 与 badge 堆叠 | 标题、按钮、卡片都带符号和胶囊标签 | 每个元素都在抢注意力 | 删除无信息作用的装饰 |
| 全局等宽界面壳 | 非代码区域也大量使用 `font-mono` | “终端感”成了与产品无关的外衣 | 等宽字体只留给代码和数据 |
| AI 文案句式 | `not just X — it's Y` | 句式很响亮，却没有提供具体事实 | 直接说明产品做什么 |

命中只是**线索**，不是判决。品牌本来就使用紫色、产品本来就是终端工具，或者 badge 确实承载版本状态时，都不应该为了“去 AI 味”而机械删除。

#### 安装与触发

为了避免实操过程占用当前课程上下文，请新开一个 Claude Code session，再直接告诉 Claude：

```text
帮我安装 kill-ai-slop，并执行：
...
```text
扫描 src/ 里的 AI slop。先给我报告，不要修改文件。
```
...
扫描会把问题按类型聚合，而不是只丢给你一个总分。下面是报告与确认环节的示意，具体格式由 Agent 宿主决定：

```text
slop  src/Hero.tsx:12  indigo→violet gradient
slop  src/Hero.tsx:31  gradient-clip headline
slop  src/Note.tsx:8   border-l-4 callout ×3
slop  copy.md:1        "not just X — it's Y"
→ 4 groups, 11 hits. 待确认：应用全部，或指定范围。
```

你应该从每条结果里看到四件事：命中位置、判断理由、建议修法、修改边界。扫描器本身不修改文件；后续由 Agent 根据你确认的范围修复。例如：

```text
只修 Hero.tsx 的渐变和 copy.md 的文案；
Note.tsx 暂时不改；保留现有品牌色。
```
...
- 不只是项目管理——这是团队协作革命。
+ 把 PR 状态、负责人和阻塞项放在一个看板里。
```

修复目标不是把页面改成另一套模板，而是去掉未经选择的默认值。已有品牌 Token、刻意保留的视觉符号和原本文案含义都应继续存在。

#### 实操练习：只修一个问题组

1. 在新的 Claude Code session 里让 Claude 安装 `kill-ai-slop`
2. 要求它扫描前端源码，并明确“先报告，不修改”
3. 从报告中选一个问题组，只批准这个 scope
4. 检查 diff 是否最小、是否保留品牌与原意
5. 再次扫描，确认目标命中已减少，其他组没有被顺手改动
...
**Q: 扫描命中渐变或 emoji，就一定要删除吗？**

A: 不一定。`kill-ai-slop` 提供的是可疑信号和解释，不是审美判决。只要它属于明确的品牌选择、真实状态或内容语义，就可以保留。

**Q: 为什么 Skill 要先报告，再让我批准 scope？**

A: 因为去除默认模板感不等于抹掉设计个性。先看理由和最小修法，再限定修改范围，才能避免自动清理误伤品牌资产。
...
- **命令与技能系统**（Lesson 6）— `kill-ai-slop` 是把审美目录封装成可复用流程的 Agent Skill
- **视觉校验**（Lesson 22.2）— 代码信号扫描之后，还要在真实页面中检查视觉结果
