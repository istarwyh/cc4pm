...
动手之前先看交付物的主体是什么。不同目标不该硬塞进同一套技术路线：

| 目标 | 默认路线 |
|------|---------|
| 验证按钮、动效、页面切换 | 使用本课的单页 HTML / React 原型 |
| 发布项目站、Docs、Book、Release 或下载页 | 使用 Lesson 24.9 的 OINK 内容站 |
| 登录、数据库、支付、实时状态 | 使用常规应用开发栈 |

判断标准很简单：**交互是主体，就做原型；内容是主体，就用 OINK；业务状态是主体，就开发应用。**不要用单文件原型硬撑几十页内容，也不要用静态站框架假装动态产品。
...
- **OINK 内容站**（Lesson 24.9）— 把项目、Docs、Book 与下载内容变成可发布静态站点
