...
> 来源：Augment Code 用自家 monorepo 数十份 `AGENTS.md`（等价于 `CLAUDE.md`）跑评测（AuggieBench），对比有/无配置文件的 Agent 输出质量，得出的系统性结论。

上面讲的是写 CLAUDE.md 的**原则**。但原则到底有没有效？ Augment Code 做了一件大多数人不会做的事——**拿真实数据验证**。

他们在生产 repo 里取出高质量 PR，还原代码环境，让 Agent 重新做一遍，然后跟"黄金 PR"对比。每个任务跑两遍：有配置文件 vs 无，单独量化文档本身的影响。

结论？**写得差的配置，比没有更糟。** 最好的文件，效果相当于把模型从 Haiku 升级到 Opus；最差的文件，让输出质量跌到连文件都没有的水平以下。

#### 六条有效模式

| # | 规律 | 核心要点 |
|---|------|---------|
| 1 | **渐进披露，不要大而全** | 100–150 行入口文件 + 按需加载的参考文档，超过即反转 |
| 2 | **步骤化工作流** | "先做第一步，再做第二步"比"这个模块是干什么的"有效得多；差距是从完不成到完成 |
| 3 | **决策表解决架构歧义** | 几种方案都说得通时，一张表让 Agent 在写第一行代码前就选对 |
| 4 | **真实代码片段 > 文字描述** | 3–10 行生产代码片段，比文字说明更能引导复用正确模式 |
| 5 | **领域特定规则有前提** | 规则明确、可执行时有效；堆了几十条则效果消失 |
| 6 | **每条"别做"配一条"做这个"** | 最稳定的规律。只有禁止项 → Agent 停下来反复核查；配替代方案 → Agent 继续往前走 |

逐条展开：

**1. 渐进披露**：研究里表现最好的配置是 100–150 行的入口文件，加上几份职责清晰的参考文档。这种组合在中等规模模块（约 100 个核心文件）里，各项评分平均提升 10–15%。文件一旦超过这个长度，收益开始反转——入口文件短，Agent 才能快速定位，而不是淹没在信息里。

**2. 步骤化工作流**：把某类任务写成带编号的多步骤工作流，是测试中提升效果最明显的模式之一。不是描述"这个模块是干什么的"，而是告诉 Agent "做这类任务时，先做第一步，再做第二步……"。有些任务上，有工作流和没有工作流的差距，是从根本完不成到顺利完成。

**3. 决策表**：代码库里有几种都说得通的方案时，决策表能帮 Agent 在写第一行代码前就做出选择，不用边写边猜。一个具体案例：React Query vs Zustand 的状态管理选型表——涉及这个模块的 PR，加了决策表后，`best_practices` 分数提升了 25%。

**4. 真实代码片段**：从生产代码里摘出来的 3–10 行真实片段，比文字说明更能引导 Agent 复用正确的模式。但要控制数量——放几个最相关、不重复的就够。片段一多，Agent 开始对着错误的样例做模式匹配。

**5. 领域特定规则**：语言习惯、组织内的特殊约定这类规则，在规则明确、可执行的情况下确实有效。问题是一旦堆了几十条，效果就消失了——原因见下方"过度探索陷阱"。

**6. "别做"配"做这个"**：写「不要直接实例化 HTTP 客户端」，Agent 看了会谨慎，但不知道该怎么办。配上「用 `lib/http` 里的 `apiClient`，它已经带了重试中间件」，Agent 知道下一步是什么，直接去做。只有禁止项的文档让 Agent 停下来反复核查，带替代方案的文档让 Agent 继续往前走。

#### 与 Lesson 5 的对照

| Lesson 5 原则 | Augment 实证 | 印证 |
|--------------|-------------|------|
| 30–100 行最佳区间 | 100–150 行最佳，超过反转 | ✓ 方向一致，Augment 给出更精确的上限 |
| "如果删掉这一行，Claude 会不会犯错" | 无用信息不只占空间，还会帮倒忙 | ✓ 信噪比越低，效果越差 |
| 六段式模板的 NEVER 段 | "别做"必须配"做这个" | ✓ 强化：NEVER 后面必须跟 ALWAYS 替代方案 |
| Rules 目录按主题拆分 | 渐进披露：入口短 + 细节推到参考文件 | ✓ 完全吻合 |

#### 过度探索陷阱

Augment 观察到最常见的失败模式——**上下文污染（context rot）**。触发方式有两种：

```
触发方式 1：架构总览写太多
→ Agent 读到大段架构描述，开始"更好地理解架构"
→ 一路打开几十个文档
→ 上下文塞进几十万 token
→ 反而写出更差的代码

触发方式 2："别做"清单太长（30–50 条禁止项，无替代方案）
→ Agent 逐条检查自己的方案是否违反每条规定
→ 翻迁移脚本、查 API 版本兼容性、读认证中间件代码
→ 把不相关的上下文全部载入
→ 交出一份做了一半的方案
```

有一个实验结果比较典型：**同一份配置文件，对不同任务产生了相反的效果**。在 bug 修复任务上，文档里的一张决策表让 `best_practices` 分数提升了 25%——Agent 看了决策表，立刻选对了方案，一气呵成。同一份文档放到复杂功能任务上，`completeness` 分数反而下降了 30%——Agent 读完决策表之后，被里面引用的参考资料吸引进去，打开了几十个 Markdown 文件逐条核查，最后创建了不必要的抽象，交了一份没做完的代码。

**对策**：你在写 CLAUDE.md 时，时刻警惕这两个陷阱。每一条规则，都问：

> "这条规则会不会让 Agent 去翻不相关的文件？"

如果会——精简它，或者把它移到按需加载的参考文件里。
