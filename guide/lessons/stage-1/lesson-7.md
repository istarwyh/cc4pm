...
- 学会用普通子代理和 `/subtask` fork 主动保护主上下文

> **互动版**：打开 [lesson-7-agent-gallery.html](lesson-7-agent-gallery.html)，浏览 18 个代理画廊、按类别筛选、查看决策树。
...
**1. 默认子代理从全新上下文开始；`/subtask` 是例外**

通常的 Explore、Plan 和自定义子代理不继承主对话历史，只接收父级发送的任务提示，因此不会被旧对话干扰。

`/subtask` 创建的是 **forked subagent**：它继承调用时的完整对话、系统提示词、模型和工具，在后台完成任务后把结果送回主对话。它适合“需要当前背景，但不值得把大量探索过程留在主上下文”的任务。详见 **Lesson 3.3**。
...
| 需要基于对话历史做独立调研 | 用 `/subtask` | fork 会继承调用时的完整对话 |
| 连续迭代同一段代码 | 不用 | 需要保持上下文连贯性 |

**记住一个原则**：任务“读很多、产出少”就用子代理；如果还需要之前的对话背景，就用 `/subtask`；需要频繁与你来回决策，则留在主对话。

### 子代理使用技巧：传递目标，而非查询

普通子代理只能看到父级发给它的提示词，看不到主对话历史（`/subtask` fork 除外）。一个常见错误是只传递字面查询，而不传递背后的目标：

```
❌ "查找所有 API 端点"
   → 子代理不知道你为什么要找，可能返回不相关的结果

✅ "我正在为 SaaS 产品写 PRD，需要了解现有 API 能力。
    查找所有 API 端点，按功能分组，标注认证方式。"
   → 子代理知道目标，返回的结果直接可用
```

**要诀**：给子代理的提示词多写一句"为什么要做这件事"，能让返回质量提升一个档次。

### 子代理的用词偏见陷阱

传递目标很重要，但还有一个更隐蔽的问题：**主 Agent 在自动分配任务时，会用不精确的用词锚定子代理的行为**。

```
你的原始请求:
  "调研小红书上关于 XX 的风评"

主 Agent 自动分配给子代理的 Prompt:
  "在小红书上搜索 XX 相关信息，总结近期风评"
                    ↑
              注意这个词！
```

即使你写了「调研」，主 Agent 在分治时会随手换成「搜索」——这直接锚定子代理去用 WebSearch 工具。但小红书是反爬平台，正确做法是用浏览器直接进站内搜索。

**用词本身就是一种隐性规则，会限制 Agent 的判断空间。**

这在多 Agent 协作中特别容易发生：

| 你的意图 | 主 Agent 可能给出的用词 | 子代理被锚定到 |
|---------|---------------------|-------------|
| 调研某平台 | "搜索相关信息" | WebSearch（可能是死路） |
| 了解竞品定价 | "查找价格" | Fetch/Grep（公开信息才有效） |
| 分析用户反馈 | "收集评论" | 爬虫思路（可能被反爬拦截） |

**怎么避免？** 在涉及多 Agent 协作的 Skill 中，需要明确提醒：

```
✅ 给子代理的 Prompt 应描述目标和约束，而非暗示具体方法：
   "获取小红书平台上关于 XX 的真实用户评价，
    注意：该平台需要浏览器访问，不支持静态抓取"

❌ 不要用暗示特定工具的动词：
   "搜索"（暗示 Search）、"抓取"（暗示 Fetch/Curl）
```

> **经验法则**：当你需要让子代理处理特定平台的任务时，**把平台的访问特征作为约束条件写入 Prompt**，而不是依赖子代理自行判断。

### 进阶：迭代检索模式

基础用法是"派子代理，拿结果"。但高级用法是**多轮交互**——编排器评估子代理的返回，不满意就追问：
...
第 2 轮（追问）：
  你: "对第 2 个问题，用子代理深入分析影响范围和修复方案"
  子代理返回: "影响 5 个文件，建议方案..."

第 3 轮（验证）：
  你: "用子代理验证这个修复方案是否会影响现有测试"
  子代理返回: "验证完毕，2 个测试需要更新..."
```

**规则**：最多 3 个周期。每轮子代理都在独立上下文中工作，不会积累前几轮的"垃圾上下文"。你在主对话中只看到精炼的摘要，但可以基于这些摘要决定是否需要下一轮深入。

## 常见问题

**Q: 子代理能看到我之前的对话吗？**

A: 普通 Explore、Plan 和自定义子代理不能，只接收父级发送的任务提示；`/subtask` 创建的 forked subagent 是例外，它继承调用时的完整对话。需要旧背景但不想让执行过程留在主线时，用 `/subtask`。
...
- 进入下一课：Lesson 7.1 - Agent Teams：多 Claude 协作团队
- 返回主菜单
- 退出学习

---
*阶段 1 | Lesson 7/26 (阶段内 7/10) | 上一课: Lesson 6.2 - Skill 链式调用 | 下一课: Lesson 7.1 - Agent Teams*
