...
在后续所有文件操作中使用解析后的绝对路径（如 `/Users/mac/.claude/sessions/`）。Write 工具调用中永远不要使用 `~` — 它可能不会被正确展开。

### 步骤 3：写入会话文件

创建 `$SESSIONS_DIR/YYYY-MM-DD-<short-id>-session.tmp`，使用今天的实际日期和一个满足 `session-manager.js` 中 `SESSION_FILENAME_REGEX` 强制规则的短 ID：
...
短 ID 规则：
- 首字符：字母（a-z、A-Z）、数字（0-9）或下划线
- 后续字符：字母、数字、下划线或连字符
- 最小长度：1 个字符（无最大限制）
- 不得以连字符开头

有效示例：`abc123de`、`emotion-pua-1`、`frontend-worktree-1`、`ChezMoi_2`
无效示例：`-starts-with-hyphen`（以连字符开头）、``（空 — 请使用旧格式）
...
```bash
ls -la “$SESSIONS_DIR/YYYY-MM-DD-<short-id>-session.tmp”
```

如果文件不存在或为空，向用户报告错误并使用绝对路径通过 Write 工具重试。
