...
仓库里同时收录了课件讲解的**参考实现**（`.claude/` 下的 170+ 个 skill、10+ 个代理、37 个命令、40+ 条规则），可与课件配合检阅，也可独立 fork 使用。基于 [Everything Claude Code](https://github.com/affaan-m/everything-claude-code)（Anthropic 黑客马拉松获奖项目）构建。
...
安装完成后在 Claude Code 里输入 `/cc4pm-guide` 启动产品主线课程。交互式安装中也可选择 `cc4lawyer-guide`；它安装的是垂直行业课程入口与法律场景课件，可通过 `npx cc4pm install --modules cc4lawyer-guide` 显式安装后运行 `/cc4lawyer-guide`。
